...
pub use value::{OwnedJsValue, JsTag, OwnedJsObject};
...
fn js_date_constructor(context: *mut q::JSContext) -> q::JSValue {
    ...
    assert_eq!(global.tag, JsTag::Object as i64);
    ...
    assert_eq!(date_constructor.tag, JsTag::Object as i64);
...
fn js_create_bigint_function(context: *mut q::JSContext) -> q::JSValue {
    ...
    assert_eq!(bigint_function.tag, JsTag::Object as i64);
    unsafe { q::JS_FreeValue(context, global) };
    bigint_function
}

/// Serialize a Rust value into a quickjs runtime value.
unsafe fn serialize_value(context: *mut q::JSContext, value: JsValue) -> Result<q::JSValue, ValueError> {
    let v = match value {
        JsValue::Undefined => q::JSValue {
            u: q::JSValueUnion { int32: 0 },
            tag: JsTag::Undefined as i64,
        },
        ...
        JsValue::String(val) => {
            ...
            if qval.tag == JsTag::Exception as i64 {
        ...
        JsValue::Array(values) => {
            ...
            if arr.tag == JsTag::Exception as i64 {
                return Err(ValueError::Internal(
        ...
        JsValue::Object(map) => {
            ...
            if obj.tag == JsTag::Exception as i64 {
        ...
        JsValue::Date(datetime) => {
            ...
            if value.tag != JsTag::Object as i64 {
                return Err(ValueError::Internal(
        ...
        JsValue::BigInt(int) => match int.inner {
            ...
            BigIntOrI64::BigInt(bigint) => {
                ...
                if js_bigint.tag != JsTag::BigInt as i64 {
                    return Err(ValueError::Internal(
...
fn deserialize_array(
    ...
    assert!(unsafe { q::JS_IsArray(context, *raw_value) } == 1);
    ...
    for index in 0..(len as usize) {
        ...
        if value_raw.tag == JsTag::Exception as i64 {
...
fn deserialize_object(context: *mut q::JSContext, obj: &q::JSValue) -> Result<JsValue, ValueError> {
    assert_eq!(obj.tag, JsTag::Object as i64);
    ...
    for index in 0..count {
        ...
        if raw_value.tag == JsTag::Exception as i64 {
        ...
        if key_value.tag == JsTag::Exception as i64 {
            return Err(ValueError::Internal(
...
//         let serialized = unsafe { serialize_value(self.context, value)? };
...
//         let global= OwnedJsValue::new(self, global_raw).try_into_object()?;
//         Ok(global)
//     }
...
//         let value = OwnedJsValue::new(self, raw);
...
//                 self.call_function_resolve(resolver, vec![obj.into_value()])?;
...
//     /// 
//     /// If the returned value is a promise, the event loop is executed until 
//     /// the promise is resolved.
//     pub fn call_function_resolve<'a>(
//         &'a self,
//         function: OwnedJsValue<'a>,
//         args: Vec<OwnedJsValue<'a>>,
//     ) -> Result<OwnedJsValue<'a>, ExecutionError> {
//         let f = function.try_into_function()?;
//         let res = f.call(args)?;
//         self.resolve_value(res)
//     }
...
//                 Ok(Ok(result)) => {
//                     let serialized = unsafe { serialize_value(context, result)? };
//                     Ok(serialized)
//                 }
...
//                     };
//                     let js_exception = unsafe { serialize_value(context, js_exception_value).unwrap() };
//                     unsafe {
...
//         if cfunc.tag != JsTag::Exception as i64 {
